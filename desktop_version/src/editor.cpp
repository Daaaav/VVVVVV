...
void editorrender(void)
{
    ...
    if(ed.boundarymod>0)
    {
        ...
        short lines;
        message = graphics.wordwrap(message, 312, &lines);
    ...
    {
        short lines;
        std::string wrapped = graphics.wordwrap(ed.textdesc, 312, &lines);
...
void editorinput(void)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                ed.keydelay = 6;
                ed.getlin(TEXT_ROOMNAME, loc::gettext("Enter new room name:"), const_cast<std::string*>(&(ed.getroomprop(ed.levx, ed.levy)->roomname)));
                game.mapheld=true;
            }
                    ...
                    if(ed.lclickdelay==0)
                    {
                        ...
                        if(tmp==-1)
                        {
                            ...
                            if(ed.drawmode==10)
                            {
                                ...
                                ed.getlin(TEXT_ROOMTEXT, loc::gettext("Enter roomtext:"), &(edentity[ed.textent].scriptname));
                            }
                        ...
                        {
                            ed.getlin(TEXT_ROOMTEXT, loc::gettext("Enter roomtext:"), &(edentity[tmp].scriptname));
                            ed.textent=tmp;
                            ed.lclickdelay=1;
                        }
