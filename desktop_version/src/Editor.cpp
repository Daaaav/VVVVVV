...
void editorrender(void)
{
    ...
    for (int i = customentities.size() - 1; i >= 0; i--)
    {
        ...
        if(customentities[i].x / 40 == ed.levx && customentities[i].y / 30 == ed.levy)
        {
            ...
            case 18: //Terminals
            {
                ...
                if (i == edent_under_cursor)
                {
                    font::print(PR_BOR | PR_CJK_HIGH, (customentities[i].x*8)- (ed.levx*40*8),(customentities[i].y*8)- (ed.levy*30*8)-8,customentities[i].scriptname,210,210,255);
                }
                break;
            }
    ...
    if(room->directmode==1)
    {
        ...
        else
        {
            ...
            FillRect(graphics.backBuffer, labellen+2, y-1, 10,10, graphics.getRGB(255 - help.glow, 196, 196));
            FillRect(graphics.backBuffer, labellen+3, y, 8,8, graphics.getRGB(0,0,0));
    ...
    else
    {
        if(ed.spacemod)
        {
            ...
            FillRect(graphics.backBuffer, 0,207-bgheight,toolnamelen+8,bgheight+1, graphics.getRGB(32,32,32));
            FillRect(graphics.backBuffer, 0,208-bgheight,toolnamelen+7,bgheight, graphics.getRGB(0,0,0));
            font::print(PR_BOR | PR_CJK_HIGH, 2,199, toolname, 196, 196, 255 - help.glow);

            char coords[8];
            vformat_buf(coords, sizeof(coords), "({x},{y})", "x:int, y:int", ed.levx+1, ed.levy+1);
            int coordslen = font::len(0, coords);
            FillRect(graphics.backBuffer, 319-coordslen-8,207-bgheight,coordslen+8,bgheight+1, graphics.getRGB(32,32,32));
            FillRect(graphics.backBuffer, 320-coordslen-8,208-bgheight,coordslen+8,bgheight, graphics.getRGB(0,0,0));
        ...
        if(ed.shiftmenu)
        {
            ...
            FillRect(graphics.backBuffer, 0,left_y-2,menuwidth+16,240, graphics.getRGB(0,0,0));
            for (size_t i = 0; i < SDL_arraysize(shiftmenuoptions); i++)
